...
class TreeNode:
    ...
    def make_leaf(self, y: pd.Series) -> None:
        ...
        self.is_leaf = True
        self.prob = one_count / total_count
...
class MyTreeClf():
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.array:
        ...
        for index in range(n):
            row = X.iloc[index]
            predictions[index] = __class__._rec_predict_proba(row, self._root)
    ...
    @staticmethod
    def _rec_predict_proba(vector: pd.Series, node: TreeNode) -> float:
        if node.is_leaf == True:
            return node.prob

        elif vector[node.feature] > node.threshold:
            return __class__._rec_predict_proba(vector, node.right)

        return __class__._rec_predict_proba(vector, node.left)
    ...
    def _print_tree_rec(node: TreeNode, side: str = None, depth: int = 0) -> None:
        if (node.is_leaf == True):
            print(f"{' ' * 3 * depth}leaf_{side} = {node.prob}")
    ...
    def get_best_column_split(self, column: pd.Series, y: pd.Series, name: str, s_0: float) -> (float, float):
        thresholds = self._get_thresholds(column, name)

        best_ig, split_value = float('-inf'), None

        for threshold in thresholds:
            ig = self.get_column_split(column, y, threshold, s_0)
    ...
    def get_gain(self, y: pd.Series) -> float:
        return getattr(self, 'get_' + self.criterion)(y)

    def get_column_split(self, column: pd.Series, y: pd.Series, threshold: float, s_0: float) -> float:
        y_1 = y[column <= threshold]
        y_2 = y[column > threshold]

        s_1 = self.get_gain(y_1)
        s_2 = self.get_gain(y_2)
    ...
    def get_entropy(y: pd.Series) -> float:
        if len(y) == 0:
            return float('inf')
        column_0 = len(y[y == 0]) / len(y)
        column_1 = len(y[y == 1]) / len(y)

        if column_0 == 0 or column_1 == 0:
            return 0

        return -(column_0 * np.log2(column_0) + column_1 * np.log2(column_1))

    @staticmethod
    def get_gini(y: pd.Series) -> float:
        if len(y) == 0:
            return float('inf')
        ...
        return 1 - (column_0 ** 2 + column_1 ** 2)